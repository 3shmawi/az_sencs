...
import 'package:az_sencs/screens/show_user_profile/show_user_profile.dart';
...
class ChatDetailsWithUser extends StatelessWidget {
  var sendMessageController = TextEditingController();
  var listController = ScrollController();

  UserModel receiverUser;

  ChatDetailsWithUser(this.receiverUser, {Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<MasterCubit, MasterStates>(
      ...
      builder: (BuildContext context, Object? state) {
        ...
        if (cubit.messagesWithUser.isNotEmpty) {
          Timer(
              const Duration(seconds: 1),
              () => listController
                  .jumpTo(listController.position.maxScrollExtent));
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            title: InkWell(
              onTap: () {
                navigateTo(context, ProfileInfoScreen(receiverUser));
              },
              child: Row(
          ...
          body: state is GetMessageLoadingState
              ...
              : cubit.messagesWithUser.isNotEmpty
                  ? Padding(
                          ...
                          Expanded(
                            child: ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  if (cubit.messagesWithUser[index].senderId ==
                                      cubit.user!.uId) {
                                    return sendMessage(
                                        cubit.messagesWithUser[index], context);
                                  } else {
                                ...
                                itemCount: cubit.messagesWithUser.length),
                          ),
                        ],
                      ),
                    )
